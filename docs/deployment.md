...
**Логин:** gociss  
**Домен:** aq-spb.ru  
**Пароль:** хранится в GitHub Secrets (см. ниже)
...
4. Добавь следующие секреты:
   ...
   - **BEGET_FTP_HOST**
     - Значение: адрес FTP сервера
     - Обычно это домен `aq-spb.ru` или специальный FTP хост (например, `ftp.beget.com`)
     - Где найти: в панели управления Бегет (https://cp.beget.com/), раздел **FTP → FTP аккаунты**
     - Если не уверен, попробуй сначала `aq-spb.ru`

   - **BEGET_FTP_USER**
     - Значение: `gociss`
     - Это логин FTP аккаунта

   - **BEGET_FTP_PASSWORD**
     - Значение: `KsZrlyR18hSg`
     - Пароль FTP аккаунта
     - ⚠️ **Важно:** Никогда не коммить этот пароль в код!
